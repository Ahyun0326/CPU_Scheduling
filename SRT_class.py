class SRT_Scheduling:
    ...
    def process_schedule(self): # CPU scheduling 작업을 수행하는 함수
        ...
        self.P_state = [[] for i in range(len(self.P_list))] # P_state 2차원 리스트를 프로세스의 개수만큼 생성
        ...
        while (self.curr_time < self.total_time):  # curr_time이 total_time보다 작다면 반복
            ...
            for i in range(self.P_execute[0]):
                ...
                self.curr_time += 1  # 한 번 실행했으므로 curr_time 1증가
                self.P_state[0] = [self.P_list[0][0], self.curr_time] # 프로세스의 이름과 현재까지의 진행 시간 저장

                if (self.curr_time != 0 and self.curr_time in self.P_arrive): # 프로세스 진행 도중 중간에 도착한 값이 있다면 해당 값 큐에 넣기
                ...
                # 타임 슬라이스만큼 실행 완료하면 큐의 마지막에 삽입 후 반복문 빠져나오기
            ...
            while (len(self.queue) > 0): # 큐가 빌 때까지 반복
                ...
                # 큐에 저장된 프로세스를 남아 있는 작업 시간이 적은 순서대로 정렬
                self.queue.sort(key=lambda x: (x[2], x[1]))

                # 큐에서 프로세스 꺼내기
                curr_process = self.queue.pop(0)

                if curr_process[0] in self.P_name: # 대기 시간 계산을 위해 프로세스 검사
                    ...
                    if curr_process[2] == self.P_execute[pos]: # 처음 실행하는 프로세스라면
                        ...
                        self.response[pos] = self.curr_time + self.response_time
                    else: # 처음 실행하는 프로세스가 아니라면
                        # 현재 도착한 시간에서 이전에 실행을 완료했던 시점의 시간을 뺀 값을 더하여 대기 시간에 저장
    ...
    def calculate_metrics(self): # 계산된 실행 시간, 응답 시간, 반환 시간을 출력하는 함수
        ...
        for i in range(len(self.turn)):
            ...
            turn_sum += self.turn[i][1]
        avg_turn = round(turn_sum / len(self.turn), 2) # 평균 반환 시간 계산

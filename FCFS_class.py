import queue #큐를 사용하기 위한 라이브러리 import
...
class FCFS_Scheduling:
    ...
    def process_schedule(self): # CPU scheduling 작업을 수행하는 함수
        ...
        for i in self.P_list:   # 큐에 도착 시간이 짧은 프로세스부터 삽입
            self.queue.put(i)
        ...
        for i in range(len(self.P_list)):   # 저장된 프로세스 수만큼 반복
            ...
            '''응답 시간 계산'''
            if arrive == 0: # FCFS의 경우, 도착 시간이 0인 프로세스는 도착 시간에 response_time을 더하여 응답시간 계산
            ...
            if arrive == 0: # 현재 할당된 프로세스의 도착시간이 0일 경우
                ...
                # 이전 프로세스가 실행을 마쳤을 때의 total에서 자신의 도착 시간을 뺀 값

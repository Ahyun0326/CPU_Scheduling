...
class SJF_Scheduling:
    ...
    def process_schedule(self): # CPU scheduling 작업을 수행하는 함수
        ...
        for i in range(len(self.P_list)): # 먼저 도착 시간이 0인 프로세스만 큐에 삽입
            if self.P_list[i][1] == '0':
                self.queue.put(self.P_list[i])
        self.P_list.sort(key=lambda x: (int(x[2]), int(x[1]))) # 실행 시간을 기준으로 오름차순 정렬, 실행 시간이 동일한 경우 먼저 도착한 프로세스부터 큐에 삽입
        ...
        for i in range(len(self.P_list)): # 저장된 프로세스 수만큼 반복
            ...
            '''응답 시간 계산'''
            if arrive == 0: # SJF의 경우, 도착 시간이 0인 프로세스는 도착 시간에 response_time을 더하여 응답시간 계산
            ...
            '''대기 시간 계산'''
            if arrive == 0: # 현재 할당된 프로세스의 도착시간이 0일 경우
